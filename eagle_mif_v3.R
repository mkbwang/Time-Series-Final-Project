...
switch(run_level, {
  ...
  eagle_Np=20000; eagle_Nmif=120; eagle_Neval=10;
  eagle_Nglobal=40; eagle_Nlocal=10
},{
...
# Iterative filtering
stew(file=sprintf("global_search_covar4_mix-%d.rda",run_level),{
  t_global <- system.time({
    mifs_global_covar4_mix <- foreach(i=1:eagle_Nglobal,
...
# log likelihood
stew(file=sprintf("lik_global_eval_covar4_mix-%d.rda",run_level),{
  t_global_eval <- system.time({
    liks_global_covar4_mix <- foreach(i=1:eagle_Nglobal,
                                       .combine=rbind, .packages='pomp',
                                       .inorder=FALSE,
                                       .options.multicore=list(set.seed=TRUE),
                                       .export=c("eagle_Neval", "eagle_2", "mifs_global_covar4_mix", "eagle_Np")) %dopar% {
                                         evals <- replicate(eagle_Neval,
                                                            logLik(pfilter(eagle_2,
                                                                           params=coef(mifs_global_covar4_mix[[i]]),Np=eagle_Np)))
